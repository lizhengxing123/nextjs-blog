...
访问类型，性能由差到好为 `all<index<range<index_merge<ref<eq_ref<const<system<NULL`
...
`const` 比较常见，只要是根据主键或者唯一索引查询，就会出现
...
-- 这个索引长度是 9，说明 post 索引没用上
mysql> explain select * from emp where age=25 and salary>2400 and post='销售';
+----+-------------+-------+------------+-------+-------------------------+-------------------------+---------+------+------+----------+-----------------------+
| id | select_type | table | partitions | type  | possible_keys           | key                     | key_len | ref  | rows | filtered | Extra                 |
+----+-------------+-------+------------+-------+-------------------------+-------------------------+---------+------+------+----------+-----------------------+
|  1 | SIMPLE      | emp   | NULL       | range | idx_emp_age_salary_post | idx_emp_age_salary_post | 9       | NULL |    1 |    10.00 | Using index condition |
+----+-------------+-------+------------+-------+-------------------------+-------------------------+---------+------+------+----------+-----------------------+
...
| id | select_type | table | partitions | type  | possible_keys           | key                     | key_len | ref  | rows | filtered | Extra                 |
+----+-------------+-------+------------+-------+-------------------------+-------------------------+---------+------+------+----------+-----------------------+
|  1 | SIMPLE      | emp   | NULL       | range | idx_emp_age_salary_post | idx_emp_age_salary_post | 76      | NULL |    3 |    10.00 | Using index condition |
+----+-------------+-------+------------+-------+-------------------------+-------------------------+---------+------+------+----------+-----------------------+
```
...
```sql
-- 索引查询
mysql> explain select * from emp where phone='13035445030';
+----+-------------+-------+------------+-------+---------------+---------------+---------+-------+------+----------+-------+
| id | select_type | table | partitions | type  | possible_keys | key           | key_len | ref   | rows | filtered | Extra |
+----+-------------+-------+------------+-------+---------------+---------------+---------+-------+------+----------+-------+
|  1 | SIMPLE      | emp   | NULL       | const | idx_emp_phone | idx_emp_phone | 46      | const |    1 |   100.00 | NULL  |
...
-- 不会使用索引
mysql> explain select * from emp where substring(phone, 8, 4)='5030';
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
| id | select_type | table | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra       |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
|  1 | SIMPLE      | emp   | NULL       | ALL  | NULL          | NULL | NULL    | NULL |   30 |   100.00 | Using where |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
```
...
```sql
-- 后面模糊匹配能使用索引
mysql> explain select * from emp where name like '荀%';
+----+-------------+-------+------------+-------+---------------+--------------+---------+------+------+----------+-----------------------+
| id | select_type | table | partitions | type  | possible_keys | key          | key_len | ref  | rows | filtered | Extra                 |
+----+-------------+-------+------------+-------+---------------+--------------+---------+------+------+----------+-----------------------+
|  1 | SIMPLE      | emp   | NULL       | range | idx_emp_name  | idx_emp_name | 66      | NULL |    2 |   100.00 | Using index condition |
+----+-------------+-------+------------+-------+---------------+--------------+---------+------+------+----------+-----------------------+

-- 前面模糊匹配回导致索引失效
mysql> explain select * from emp where name like '%娘';
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
| id | select_type | table | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra       |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
|  1 | SIMPLE      | emp   | NULL       | ALL  | NULL          | NULL | NULL    | NULL |   30 |    11.11 | Using where |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
```
...
| id | select_type | table | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra       |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
|  1 | SIMPLE      | emp   | NULL       | ALL  | idx_emp_phone | NULL | NULL    | NULL |   30 |    10.00 | Using where |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
```

#### or两侧用到的列，只要有一侧没用到索引，整个索引都会失效

```sql
-- 由于 leader_id 没有索引，所以 name 索引也会用不上
mysql> explain select * from emp where name='张飞' or leader_id='1';
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
| id | select_type | table | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra       |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
|  1 | SIMPLE      | emp   | NULL       | ALL  | idx_emp_name  | NULL | NULL    | NULL |   30 |    13.00 | Using where |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+

-- 解决办法就是给 leader_id 创建个索引
mysql> create index idx_emp_leader on emp(leader_id);
mysql> explain select * from emp where name='张飞' or leader_id='1';
+----+-------------+-------+------------+-------------+-----------------------------+-----------------------------+---------+------+------+----------+-------------------------------------------------------+
| id | select_type | table | partitions | type        | possible_keys               | key                         | key_len | ref  | rows | filtered | Extra                                                 |
+----+-------------+-------+------------+-------------+-----------------------------+-----------------------------+---------+------+------+----------+-------------------------------------------------------+
|  1 | SIMPLE      | emp   | NULL       | index_merge | idx_emp_name,idx_emp_leader | idx_emp_name,idx_emp_leader | 66,5    | NULL |    5 |   100.00 | Using union(idx_emp_name,idx_emp_leader); Using where |
+----+-------------+-------+------------+-------------+-----------------------------+-----------------------------+---------+------+------+----------+-------------------------------------------------------+

-- 如果使用联合索引，但不是最左列的话，也会导致索引失效
mysql> explain select * from emp where phone='13035445021' or salary=8000;
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
| id | select_type | table | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra       |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
|  1 | SIMPLE      | emp   | NULL       | ALL  | idx_emp_phone | NULL | NULL    | NULL |   30 |    13.00 | Using where |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+

-- 使用最左列的话就不会导致索引失效了
mysql> explain select * from emp where phone='13035445021' or age=30;
+----+-------------+-------+------------+-------------+---------------------------------------+---------------------------------------+---------+------+------+----------+----------------------------------------------------------------------+
| id | select_type | table | partitions | type        | possible_keys                         | key                                   | key_len | ref  | rows | filtered | Extra                                                                |
+----+-------------+-------+------------+-------------+---------------------------------------+---------------------------------------+---------+------+------+----------+----------------------------------------------------------------------+
|  1 | SIMPLE      | emp   | NULL       | index_merge | idx_emp_phone,idx_emp_age_salary_post | idx_emp_phone,idx_emp_age_salary_post | 46,4    | NULL |    3 |   100.00 | Using sort_union(idx_emp_phone,idx_emp_age_salary_post); Using where |
+----+-------------+-------+------------+-------------+---------------------------------------+---------------------------------------+---------+------+------+----------+----------------------------------------------------------------------+
```

#### 数据分部影响

只要查询的数据占了大部分，mysql就会放弃索引走全表扫描

```sql
mysql> explain select * from emp where phone > '13035445000';
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
| id | select_type | table | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra       |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
|  1 | SIMPLE      | emp   | NULL       | ALL  | idx_emp_phone | NULL | NULL    | NULL |   30 |   100.00 | Using where |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
```
...
| id | select_type | table | partitions | type | possible_keys                       | key                     | key_len | ref   | rows | filtered | Extra |
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------+------+----------+-------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_age_salary_post,idx_emp_age | idx_emp_age_salary_post | 4       | const |    3 |   100.00 | NULL  |
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------+------+----------+-------+

-- 指定使用 age 索引
mysql> explain select * from emp use index(idx_emp_age) where age = 25;
+----+-------------+-------+------------+------+---------------+-------------+---------+-------+------+----------+-------+
| id | select_type | table | partitions | type | possible_keys | key         | key_len | ref   | rows | filtered | Extra |
+----+-------------+-------+------------+------+---------------+-------------+---------+-------+------+----------+-------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_age   | idx_emp_age | 4       | const |    3 |   100.00 | NULL  |
+----+-------------+-------+------------+------+---------------+-------------+---------+-------+------+----------+-------+
```
...
```sql
mysql> explain select * from emp ignore index(idx_emp_age) where age = 25;
+----+-------------+-------+------------+------+-------------------------+-------------------------+---------+-------+------+----------+-------+
| id | select_type | table | partitions | type | possible_keys           | key                     | key_len | ref   | rows | filtered | Extra |
+----+-------------+-------+------------+------+-------------------------+-------------------------+---------+-------+------+----------+-------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_age_salary_post | idx_emp_age_salary_post | 4       | const |    3 |   100.00 | NULL  |
...
```sql
mysql> explain select * from emp force index(idx_emp_age) where age = 25;
+----+-------------+-------+------------+------+---------------+-------------+---------+-------+------+----------+-------+
| id | select_type | table | partitions | type | possible_keys | key         | key_len | ref   | rows | filtered | Extra |
+----+-------------+-------+------------+------+---------------+-------------+---------+-------+------+----------+-------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_age   | idx_emp_age | 4       | const |    3 |   100.00 | NULL  |
...
查询的时候使用了索引，而且需要返回的列都在该索引里面，不需要再回表查询，[详细解释](https://www.bilibili.com/video/BV1t34y1u7Kg/?spm_id_from=pageDriver&vd_source=97e4871747b6e43793eaa0ddb1bb5191)

```sql
-- 使用索引的时候，下面也挂了一个 id 字段
-- 这里需要回表查询其他字段
mysql> explain select * from emp where age=25 and salary=2400 and post='销售';
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+-----------------------+
| id | select_type | table | partitions | type | possible_keys                       | key                     | key_len | ref               | rows | filtered | Extra                 |
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+-----------------------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_age_salary_post,idx_emp_age | idx_emp_age_salary_post | 76      | const,const,const |    2 |   100.00 | Using index condition |
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+-----------------------+

-- 这里需要回表查询 name 字段
mysql> explain select id, name, age from emp where age=25 and salary=2400 and post='销售';
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+-----------------------+
| id | select_type | table | partitions | type | possible_keys                       | key                     | key_len | ref               | rows | filtered | Extra                 |
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+-----------------------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_age_salary_post,idx_emp_age | idx_emp_age_salary_post | 76      | const,const,const |    2 |   100.00 | Using index condition |
...
-- 这里不需要回表查询
mysql> explain select id, salary, age from emp where age=25 and salary=2400 and post='销售';
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+--------------------------+
| id | select_type | table | partitions | type | possible_keys                       | key                     | key_len | ref               | rows | filtered | Extra                    |
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+--------------------------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_age_salary_post,idx_emp_age | idx_emp_age_salary_post | 76      | const,const,const |    2 |   100.00 | Using where; Using index |
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+--------------------------+

-- 这里不需要回表查询
mysql> explain select id, salary, age, post from emp where age=25 and salary=2400 and post='销售';
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+--------------------------+
| id | select_type | table | partitions | type | possible_keys                       | key                     | key_len | ref               | rows | filtered | Extra                    |
+----+-------------+-------+------------+------+-------------------------------------+-------------------------+---------+-------------------+------+----------+--------------------------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_age_salary_post,idx_emp_age | idx_emp_age_salary_post | 76      | const,const,const |    2 |   100.00 | Using where; Using index |
...
在给长文本字段创建索引的时候，我们可以使用前缀索引，就是只使用字符串的部分前缀来创建索引，这样可以节约很多索引空间，同事在索引的时候，磁盘io也会降低很多

```sql
create index 索引名 on 表名(字段名(截取的字符个数));
```

> 索引的选择性：不重复的索引值，和总记录数的比值，这个值越大，说明索引选择性越高，查询效率越高

计算选择性

```sql
mysql> select count(distinct substring(email, 1, 10))/count(id) from emp;
+---------------------------------------------------+
| count(distinct substring(email, 1, 10))/count(id) |
+---------------------------------------------------+
|                                            1.0000 |
+---------------------------------------------------+

mysql> select count(distinct substring(email, 1, 9))/count(id) from emp;
+--------------------------------------------------+
| count(distinct substring(email, 1, 9))/count(id) |
+--------------------------------------------------+
|                                           0.9667 |
+--------------------------------------------------+

mysql> select count(distinct substring(email, 1, 4))/count(id) from emp;
+--------------------------------------------------+
| count(distinct substring(email, 1, 4))/count(id) |
+--------------------------------------------------+
|                                           0.9667 |
+--------------------------------------------------+

mysql> select count(distinct substring(email, 1, 3))/count(id) from emp;
+--------------------------------------------------+
| count(distinct substring(email, 1, 3))/count(id) |
+--------------------------------------------------+
|                                           0.8667 |
+--------------------------------------------------+
```
...
```sql
-- 创建索引
mysql> create index idx_emp_email_4 on emp(email(4));

mysql> show index from emp;
+-------+------------+-------------------------+--------------+-------------+-----------+-------------+----------+--------+------+------------+---------+---------------+---------+------------+
| Table | Non_unique | Key_name                | Seq_in_index | Column_name | Collation | Cardinality | Sub_part | Packed | Null | Index_type | Comment | Index_comment | Visible | Expression |
+-------+------------+-------------------------+--------------+-------------+-----------+-------------+----------+--------+------+------------+---------+---------------+---------+------------+
| emp   |          1 | idx_emp_email_4         |            1 | email       | A         |          29 |        4 |   NULL | YES  | BTREE      |         |               | YES     | NULL       |
+-------+------------+-------------------------+--------------+-------------+-----------+-------------+----------+--------+------+------------+---------+---------------+---------+------------+

-- 查看执行计划
mysql> explain select * from emp where email='xabcdefghijk@qq.com';
+----+-------------+-------+------------+------+-----------------+-----------------+---------+-------+------+----------+-------------+
| id | select_type | table | partitions | type | possible_keys   | key             | key_len | ref   | rows | filtered | Extra       |
+----+-------------+-------+------------+------+-----------------+-----------------+---------+-------+------+----------+-------------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_emp_email_4 | idx_emp_email_4 | 19      | const |    1 |   100.00 | Using where |
+----+-------------+-------+------------+------+-----------------+-----------------+---------+-------+------+----------+-------------+
```

### 索引总结

- 如果频繁的出现了多个查询条件，就要考虑给这些查询字段创建联合索引，而不是单列索引
- 对于数据量较大，并且查询比较频繁的表，我们就应该为它创建索引
- 一般都是选择查询条件、排序条件、或者分组条件用到的字段来创建索引
- 选择字段的时候，要尽量选择区分度高的列作为索引，这样效率比较高
- 尽量创建联合索引，其在很多场景下可以覆盖索引，效率更高，但要注意字段顺序
- 字符串较长的话，可以建立前缀索引，以此来降低索引的体积，减少磁盘io，提高效率
- 索引的数量并不是越多越好，索引越多，优点是提高查询效率，缺点就是在增删改的时候都需要维护索引，效率降低，占用的体积也很大
- 如果一个字段不能为null，我们就应该加上 not null 的约束，有了这个约束后，优化器可以更精确的估计查询条件的过滤效果，并且制定更准确的查询执行计划，更快的返回结果